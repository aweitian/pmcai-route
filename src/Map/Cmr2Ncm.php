...
use Aw\Routing\Matcher\Ca as CaMatcher;
use Aw\Routing\Matcher\Mca as McaMatcher;
use Aw\Routing\Matcher\Regexp as RegexpMatcher;

class Cmr2Ncm implements INcm
{
    ...
    {
        if ($matcher instanceof CaMatcher) {
            $this->matcher_which = 'ca';
        } else if ($matcher instanceof McaMatcher) {
            $this->matcher_which = 'mca';
        } else if ($matcher instanceof RegexpMatcher) {
    ...
    public function filter()
    {
        ...
        foreach ($ns as $k => $item) {
            if (!preg_match($pattern, $item)) {
                ...
                if (preg_match($digital_pattern, $tmp)) {
                    $tmp = Cmr2Ncm::DIGITAL_PREFIX . $tmp;
                }
                $ns[$k] = $tmp;
            }
        }
        ...
        if (!preg_match($pattern, $this->class)) {
            ...
            if (preg_match($digital_pattern, $this->class)) {
                $this->class = Cmr2Ncm::DIGITAL_PREFIX . $this->class;
            }
        }
        ...
        if (!preg_match($pattern, $this->method)) {
            ...
            if (preg_match($digital_pattern, $this->method)) {
                $this->method = Cmr2Ncm::DIGITAL_PREFIX . $this->method;
            }
        }
    }
